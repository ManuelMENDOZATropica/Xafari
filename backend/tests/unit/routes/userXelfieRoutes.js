...
describe("UserXelfie route", () => {
  // describe("GET UserXelfie", () => {
  ...
  describe("POST UserXelfie create", () => {
    it("Should add a new UserXelfie", async () => {
      const user = {
        name: Math.random().toString(16).substring(2),
        email: `${Math.random().toString(16).substring(2)}@gmail.com`,
        password: "1234",
      ...
      const response = await request(app)
        .post(`/user/${responseUser.body.user.id}/xelfie`)
        .send({
          xelfieId: responseXelfie.body.xelfie.id,
          xelfieUrl: "www.image.com",
        });
      ...
      console.debug(responseGet.body.user.activities[0].xelfie);
    });
  });
