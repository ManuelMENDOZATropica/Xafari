...
export default function PodiumModal({ onClose }) {
  ...
  return (
    <motion.div className="relative h-full w-full overflow-y-auto bg-[url('/img/fondoArbolDeLaVida.png')] bg-cover bg-center px-4 py-4 font-apercu md:px-10">
      ...
      <div className="mx-auto mb-6 flex max-w-md items-center gap-4 rounded-2xl border border-gray-300 bg-white/90 p-4 shadow-md backdrop-blur-md">
        ...
        <div className="flex w-full flex-col">
          ...
          <div className="mt-1">
            ...
            <progress value={userProgress.xperiencias || 0} max="100" className="h-2 w-full rounded bg-gray-200 [&::-webkit-progress-bar]:bg-gray-200 [&::-webkit-progress-value]:bg-emerald-600" />
          </div>
          <div className="mt-1">
            <p className="text-xs text-gray-700">{t("checklistLabel")}</p>
            <progress value={userProgress.checklist || 0} max="100" className="h-2 w-full rounded bg-gray-200 [&::-webkit-progress-bar]:bg-gray-200 [&::-webkit-progress-value]:bg-blue-500" />
          </div>
          <div className="mt-1">
            <p className="text-xs text-gray-700">{t("secretsLabel")}</p>
            <progress value={userProgress.xecretos || 0} max="100" className="h-2 w-full rounded bg-gray-200 [&::-webkit-progress-bar]:bg-gray-200 [&::-webkit-progress-value]:bg-purple-500" />
          </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-2 gap-4 max-w-5xl mx-auto pb-10">
        {top10.map((jugador, idx) => (
          <div
            key={idx}
            className={`flex items-center gap-4 bg-white/90 backdrop-blur-md rounded-2xl p-4 shadow-md transition-transform relative ${borderClass(idx)}`}
          >
            <div className="relative w-20 h-20 rounded-full overflow-hidden shadow-md shrink-0">
              <AvatarRender avatarData={jugador.avatarData} className="w-full h-full object-cover" />
            </div>
            <div className="flex flex-col w-full">
              <p className="text-lg font-bold text-gray-800">{jugador.nombre}</p>
              <div className="mt-1">
                <p className="text-xs text-gray-700">{t("xperiencesLabel")}</p>
                <progress value={jugador.detalle.xperiencias} max="100" className="w-full h-2 rounded bg-gray-200 [&::-webkit-progress-bar]:bg-gray-200 [&::-webkit-progress-value]:bg-emerald-600" />
              </div>
              <div className="mt-1">
                <p className="text-xs text-gray-700">{t("checklistLabel")}</p>
                <progress value={jugador.detalle.checklist} max="100" className="w-full h-2 rounded bg-gray-200 [&::-webkit-progress-bar]:bg-gray-200 [&::-webkit-progress-value]:bg-blue-500" />
              </div>
              <div className="mt-1">
                <p className="text-xs text-gray-700">{t("secretsLabel")}</p>
                <progress value={jugador.detalle.xecretos} max="100" className="w-full h-2 rounded bg-gray-200 [&::-webkit-progress-bar]:bg-gray-200 [&::-webkit-progress-value]:bg-purple-500" />
              </div>
            </div>
          </div>
        ))}
